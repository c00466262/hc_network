...
void disable_mcg_stop_enables(void)
{
  // Must disable Oscillator stop enable; otherwise oscillator will continue
  //     to run in ALL low power modes
  OSC_CR &= ~OSC_CR_EREFSTEN_MASK;
  
  // Must disable PLL stop enable;  otherwise PLL will run in stop mode
  MCG_C5 &= ~MCG_C5_PLLSTEN_MASK;
  
  // Must disable IREF stop enable; otherwise the internal reference clock
  //  will continue to run in stop mode
